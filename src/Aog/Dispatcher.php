...
final class Dispatcher
{
    ...
    {
        $pcuid=(string)($form['pcuid']??'GUEST');$m=$this->db->getMatch($pcuid)??[];$gmode=(int)($m['gmode']??1);if(!isset(self::GMODE_TAKU[$gmode]))$gmode=1;$taku=self::GMODE_TAKU[$gmode];$seats=(int)($m['seats']??self::SEATS[$taku]);$table=is_array($m['table']??null)?$m['table']:null;$hasTable=$table!==null;
        if($hasTable){$table['state']='game_end';$table['finished']=true;$scores=array_values(array_map('intval',$table['scores']??[]));$kyoku=(int)($table['kyoku_index']??0);}else{$scores=array_fill(0,$seats,self::START_SCORE[$taku]);$kyoku=0;}
        while(count($scores)<$seats)$scores[]=self::START_SCORE[$taku];
        if($hasTable){$oya=$seats>0?$kyoku%$seats:0;$order=range(0,max(0,$seats-1));usort($order,static function(int $a,int $b)use($scores,$oya,$seats):int{$cmp=$scores[$b]<=>$scores[$a];if($cmp!==0)return $cmp;$wa=($a-$oya+$seats)%$seats;$wb=($b-$oya+$seats)%$seats;return $wa<=>$wb;});$ranks=array_fill(0,4,0);foreach($order as $rank=>$seat)$ranks[$seat]=$rank;$umaTable=[4=>[20000,10000,-10000,-20000],3=>[20000,0,-20000],2=>[10000,-10000]];$uma=$umaTable[$seats]??array_fill(0,$seats,0);}else{$ranks=[0,1,2,3];$uma=array_fill(0,$seats,0);}
        ...
        $m['state']='game_end';if($hasTable)$m['table']=$table;$this->db->saveMatch($pcuid,$m);return $this->xml($body);
    }
