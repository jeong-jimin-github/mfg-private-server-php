...
final class Dispatcher
{
    ...
    private function gget(array $form): string
    {
        ...
        if($ready){if($table===null)$table=new Table(Table::create((int)$match['taku'],0));if(($match['state']??'')==='matching'){$table->startKyoku();$match['state']='playing';}$table->flushPending();$match['table']=$table->state();$this->db->saveMatch($pcuid,$match);}
