...
p = Path('src/Eamuse/Dispatcher.php')
s = p.read_text()
old = "$module === 'vfglog' => $this->wrap('vfglog', ''),"
new = "$module === 'vfglog' => $this->vfglog($method, $root),"
if old not in s:
    raise SystemExit('vfglog dispatch anchor missing')
s = s.replace(old, new, 1)
anchor = "    private function findModuleNode(?SimpleXMLElement $root, string $name): ?SimpleXMLElement\n"
method = """    private function vfglog(string $method, ?SimpleXMLElement $root): string
    {
...
"""
if anchor not in s:
    raise SystemExit('vfglog insertion anchor missing')
s = s.replace(anchor, method + anchor, 1)
p.write_text(s)

test = Path('tests/vfglog_test.php')
